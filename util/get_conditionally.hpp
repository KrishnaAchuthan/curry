...
CONSTEXPR decltype(auto) decorate_arg(std::integral_constant<ArgCategory, ArgCategory::Function>, A&& a) {
   return std::make_tuple(int_list<I>(), std::forward<decltype(a._f)>(a._f));
}
...
CONSTEXPR decltype(auto) get_conditionally_impl1(std::true_type, int_list<ArgPosition>, A&& a) {
   return decorate_arg<I>(arg_category<decltype(std::get<ArgPosition + I - 1>(std::forward<A>(a)))>(), std::get<ArgPosition + I - 1>(std::forward<A>(a)));
}
